<script lang="ts">
	...
	$effect(() => {
		...
		if (
			radar_state.radar_state.timestamp !== lastTimestamp &&
			layers_state.data.radar_layer === true
		) {
...
<div class="absolute left-0 top-0 h-full w-full" bind:this={mapElement}></div>
