...
std::string extract_coded(const std::string &url)
{
...
void write_mosaic_descriptor_files(const std::string &directory_path)
{
    std::ofstream indexer(directory_path + "/indexer.properties");
...
void load_all()
{
    ...
    {
        if (!result.was_fetched || !result.is_fresh)
            continue;

        try
        {
            std::string code = extract_coded(url);
            ...
            Visualizer::save_image(sweep, filename, lat, lon, code);
        }
...
{
    Stations::load_csv("data/nexrad_stations.csv");
    auto [lat, lon] = Stations::get("KENX");

    auto raw = FTP::download_single("ftp://tgftp.nws.noaa.gov/SL.us008001/DF.of/DC.radar/DS.p94r0/SI.kenx/sn.last");
    auto decompressed = parser::decompress_bzip2(raw);
    parser::RadarSweep sweep = parser::parse_packet_16(decompressed);
    Visualizer::save_image(sweep, "outputs/radar.tif", lat, lon, "KENX");
}
...
int main()
{
    ...
    // write_mosaic_descriptor_files("outputs");
