...
{   /* BEGIN Set_Mercator_Parameters */
    ...
    { /* no errors */
        ...
        Merc_Scale_Factor = 1.0 / (sqrt(1.e0 - Merc_es * sin_olat * sin_olat) / cosf(Origin_Latitude));
        es2 = Merc_es * Merc_es;
        es3 = es2 * Merc_es;
        es4 = es3 * Merc_es;
        Merc_ab = Merc_es / 2.e0 + 5.e0 * es2 / 24.e0 + es3 / 12.e0 + 13.e0 * es4 / 360.e0;
...
{   /* BEGIN Convert_Geodetic_To_Mercator */
    ...
    { /* no errors */
        ...
        *Northing = Merc_Scale_Factor * Merc_a * logf(ctanz2) + Merc_False_Northing;
        ...
        if (Delta_Long < -M_PI)
            Delta_Long += (TWO_PI);
        *Easting = Merc_Scale_Factor * Merc_a * Delta_Long + Merc_False_Easting;
    }
    return (Error_Code);
...
{   /* BEGIN Convert_Mercator_To_Geodetic */
    ...
    { /* no errors */
        ...
        *Longitude = Merc_Origin_Long + dx / (Merc_Scale_Factor * Merc_a);
        xphi = PI_OVER_2 - 2.e0 * atanf(1.e0 / exp(dy / (Merc_Scale_Factor * Merc_a)));
        *Latitude = xphi + Merc_ab * sinf(2.e0 * xphi) + Merc_bb * sinf(4.e0 * xphi) + Merc_cb * sinf(6.e0 * xphi) + Merc_db * sinf(8.e0 * xphi);
        if (*Longitude > M_PI)
            *Longitude -= (TWO_PI);
        if (*Longitude < -M_PI)
            *Longitude += (TWO_PI);
